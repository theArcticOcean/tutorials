...
#else
	#ifndef GW_ASSERT
    #define GW_ASSERT(expr)	if(!(expr)) cerr << "Error in file " << __FILE__ << " line " << __LINE__ << "." <<std::endl 
  #endif
...
//		GW_ASSERT(GW_False);
		if( (bit&_SW_UNDERFLOW)==_SW_UNDERFLOW )
			cerr << "Math operation problem : underflow." <<std::endl;
		if( (bit&_SW_OVERFLOW)==_SW_OVERFLOW )
			cerr << "Math operation problem : overflow." <<std::endl;
		if( (bit&_SW_ZERODIVIDE)==_SW_ZERODIVIDE )
			cerr << "Math operation problem : zero divide." <<std::endl;
		if( (bit&_SW_INVALID)==_SW_INVALID )
			cerr << "Math operation problem : invalid." <<std::endl;	
	}
#endif
}
